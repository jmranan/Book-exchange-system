            ...
            if temp_1(all_uesrs_id, all_password_md5, uesr_education_id_input) == hashmd5(uesr_password_input):
                ...
                for a, i in enumerate(callbacka):
                    if i["education_id"] != uesr_education_id_input:
                        ...
                        # os.system("pause")
                print("-----------------------------亲，已经到底了！--------------------------------------\n\n")
                xid = input("请输入您需要与他（她）的哪三本书进行交换，从新启动程序请输入e（输入序号）：")
                os.system("cls")
                if xid == "e" or xid == "E":
                    print("正在从新启动程序...")
                    time.sleep(2)
                ...
                for a, i in enumerate(callbacka):
                    if not i["education_id"] != uesr_education_id_input:
                        ...
                        # os.system("pause")
                print("-----------------------------亲，已经到底了！--------------------------------------\n\n")
                yid = input("请输入您要将您的哪三本书与他（她）交换，从新启动程序请输入e（输入序号）：")
                os.system("cls")
                if yid == "e" or yid == "E":
                    print("正在从新启动程序...")
                    time.sleep(2)
                ...
                os.system("pause")
                while True:
                    xxx = input("是否成交，是请输入y，否请输入n：")
                    if xxx == "y" or xxx == "Y":
                        ye = int(input("请输入您和他（她）约好的归还日期（年）："))
                        mo = int(input("请输入您和他（她）约好的归还日期（月）："))
                        da = int(input("请输入您和他（她）约好的归还日期（日）："))
                        print("正在执行指令...")
                        ...
                        exeb = f"""insert into borrowbookslist (a_education_id, b_education_id, a_borrowbook_one, a_borrowbook_two, a_borrowbook_three, b_borrowbook_one, b_borrowbook_two, b_borrowbook_three, a_PhoneNumber, b_PhoneNumber, create_date, ok_date) values ({callbacka[xid]["education_id"]},{callbacka[yid]["education_id"]},"{callbacka[xid]["borrowbook_one"]}","{callbacka[xid]["borrowbook_two"]}","{callbacka[xid]["borrowbook_three"]}","{callbacka[yid]["borrowbook_one"]}","{callbacka[yid]["borrowbook_two"]}","{callbacka[yid]["borrowbook_three"]}",{callbacka[xid]["PhoneNumber"]},{callbacka[yid]["PhoneNumber"]},date('{now_date}'),date('{datetime.date(ye, mo, da)}')); """
                        mysqlp(host=host, uesr=uesr, password=password, db=db, exe=exeb, UPs=UsingPassword)
                        print("执行成功")
                        os.system("pause")
                        break
                    elif xxx == "n" or xxx == "N":
            ...
            if temp_1(all_uesrs_id, all_password_md5, uesr_education_id_input) == hashmd5(uesr_password_input):
                for rd in QUOB(uesr_education_id_input):
                    if rd["ok_date"].year >= now_date.year:
                        mysqlp(host=host, uesr=uesr, password=password, db=db,exe=f"""delete from borrowbookslist where id={rd["id"]}""")
